...
@groups_only_response
@group_admin_only
def kick_callback(bot, update, args):
    chat_id = update.effective_chat.id
    user = update.effective_user
    if update.effective_message.reply_to_message:
        to_user = update.effective_message.reply_to_message.from_user
        ...
        if not args:
            update.effective_message.reply_text(_("Reply to someone or use `/kick username/id`."),
                                                parse_mode=ParseMode.MARKDOWN)
            return
        else:
            try:
    ...
    if if_group_admin(bot, chat_id, db_user.telegramid):
        ...
        bot.unban_chat_member(chat_id, db_user.telegramid)
        update.effective_message.reply_text(
            _("{0} (`{1}`) has been kicked by {2}.").format(db_user.markdown_first,
                                                            db_user.telegramid,
                                                            user.mention_markdown(user.first_name)),
            parse_mode=ParseMode.MARKDOWN)
        return
...
@groups_only_response
@group_admin_only
def unban_callback(bot, update, args):
    chat_id = update.effective_chat.id
    user = update.effective_user
    if update.effective_message.reply_to_message:
        to_user = update.effective_message.reply_to_message.from_user
        ...
        if not args:
            update.effective_message.reply_text(_("Reply to someone or use `/unban username/id`."),
                                                parse_mode=ParseMode.MARKDOWN)
            return
        else:
            try:
    ...
    if if_group_admin(bot, chat_id, db_user.telegramid):
        ...
        status = bot.get_chat_member(chat_id, db_user.telegramid).status
        if status == 'left' or status == 'kicked':
            bot.unban_chat_member(chat_id, db_user.telegramid)
            update.effective_message.reply_text(
                _("{0} (`{1}`) has been unbanned by {2}.").format(db_user.markdown_first,
                                                                  db_user.telegramid,
                                                                  user.mention_markdown(user.first_name)),
                parse_mode=ParseMode.MARKDOWN)
        else:
            update.effective_message.reply_text(
                _("{0} (`{1}`) was not banned...").format(db_user.markdown_first,
                                                          db_user.telegramid,
                                                          user.mention_markdown(user.first_name)),
                parse_mode=ParseMode.MARKDOWN)
        return
...
def register():
    ...
    dispatcher.add_handler(CommandHandler("kick", kick_callback, pass_args=True))
    dispatcher.add_handler(CommandHandler("tempban", tempban_callback, pass_args=True,
                                          pass_job_queue=True))
    dispatcher.add_handler(CommandHandler("unban", unban_callback, pass_args=True))
